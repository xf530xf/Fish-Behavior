...
def parse_args():
    ...
    parser.add_argument('--out', type=str, help='Output video file')
    parser.add_argument('--save_dir', type=str, help='Output for video frames')
    parser.add_argument('--plot_dir', type=str, default='plots', help='Output directory for plots') # Added plot_dir
    parser.add_argument('--texts', help='text prompt')
    parser.add_argument('--line_width', type=int, default=5, help='Line width')
    parser.add_argument('--unified', action='store_true', help='Use unified model, which means the masa adapter is built upon the detector model.')
    parser.add_argument('--detector_type', type=str, default='mmdet', help='Choose detector type')
...
def main():
    ...
    frames = []
    fps_list = []
    # Dictionary to store the trajectory of each instance
    ...
    for frame in track_iter_progress((video_reader, len(video_reader))):
        ...
        # Extract bounding box centers and track IDs
        bboxes = track_result[0].pred_track_instances.bboxes.cpu().numpy()
        track_ids = track_result[0].pred_track_instances.instances_id.cpu().numpy()

        # Calculate the center of each bounding box and store it in the dictionary
        for i, track_id in enumerate(track_ids):
            bbox = bboxes[i]
            center_x = (bbox[0] + bbox[2]) / 2
            center_y = (bbox[1] + bbox[3]) / 2

            if track_id not in instance_trajectories:
                instance_trajectories[track_id] = []
            instance_trajectories[track_id].append((center_x, center_y))
    ...
    # Create plots for each instance trajectory
    for track_id, trajectory in instance_trajectories.items():
        x_coords, y_coords = zip(*trajectory)  # Unpack the list of tuples into two lists
        plt.figure(figsize=(10, 6))  # Set the figure size
        plt.plot(x_coords, y_coords, marker='o', linestyle='-', color='b')  # Plot the trajectory
        plt.xlabel('X Coordinate')  # Label the x-axis
        plt.ylabel('Y Coordinate')  # Label the y-axis
        plt.title(f'Trajectory of Instance {track_id}')  # Set the title of the plot
        plt.grid(True)  # Add a grid to the plot

        # Save the plot to a file
        plot_filename = os.path.join(args.plot_dir, f'trajectory_{track_id}.png')
